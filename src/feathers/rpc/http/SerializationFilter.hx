...
class SerializationFilter {
	...
	private static var filterForResultFormatTable:Dynamic = {};
	...
	**/
	public static function registerFilterForResultFormat(resultFormat:String, filter:SerializationFilter):SerializationFilter {
		...
		Reflect.setField(filterForResultFormatTable, resultFormat, filter);
		return old;
	}
	...
	**/
	public function serializeParameters(operation:AbstractOperation, params:Array<Dynamic>):Dynamic {
		...
		if (argNames == null || params.length != argNames.length)
			throw new ArgumentError("HTTPMultiService operation called with "
				+ (argNames == null ? 0 : argNames.length)
				+ " argumentNames and "
				+ params.length
				+ " number of parameters.  When argumentNames is specified, it must match the number of arguments passed to the invocation");

		var obj:Dynamic = {};
		for (i in 0...argNames.length)
			Reflect.setField(obj, argNames[i], params[i]);
